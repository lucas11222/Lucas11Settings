...
void toggleSpotifyVolume(float volume)
{
    std::wstring targetExeName = Mod::get()->template getSettingValue<std::filesystem::path>("spotifyApp").filename().wstring();
    DWORD processId = GetProcessIdByName(targetExeName);
    if (processId)
